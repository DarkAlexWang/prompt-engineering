# Prompt Engineering: Self Practice Example

This repository contains a self practice example to show how to do prompt engineering with multiple different techniques, and the chat given are some example customer and service conversation.
The goal is to:
1. Reformat the date and time to date only.
2. Reformat the customer and service role header.
3. Hide all personal information.
4. Replace curse words to emoji.
The project allows you to preprocess chats using GPT-3.5 and GPT-4 using the OpenAI API. And I also replace the OpenAI API with gptapi.us API for cheaper cost.
...
Create and activate a virtual environment (conda create env prompt). Then install the `openai` dependency:
...
Change the prompts used in the script by editing the entries in [`settings.toml`](settings.toml). The repository also contains a second settings file, [`settings-final.toml`](settings-final.toml), which contains all the final prompts that been used.
